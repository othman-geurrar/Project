...
const LifestyelDetails = () => {
  ...
  return (
    <div className='grid  md:grid-cols-3 md:grid-rows-12 md:gap-10 gap-6 p-6 h-screen'>

        {/* Image Section */}
        <div className='mt-10 md:col-span-3 md:row-span-8 grid-s rounded-md '>
            <img className=' object-fill rounded-md w-full h-full md:object-cover md:object-left-top'
             src={`${data?.ImageURL}`} alt="Lifestyle image" />
        </div>
        ...
        <h1 className=' text-2xl font-bold mb-4'> {data?.LifeStyleName} </h1>
        <h3 className=' font-semibold text-lg'> {data?.styleType} </h3>
        <p className='mt-4 font-sans text-lg'> {data?.Content.description} </p>
        ...
        <div className='md:col-span-1 md:row-span-4  '>
            <h1 className='text-2xl font-bold font-mono mb-4 text-[#63cade]'> Up Coming Events </h1>
